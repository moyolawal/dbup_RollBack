...
namespace DbUp.Support
{
    ...
    public abstract class TableJournal : IJournal
    {
        ...
        /// </summary>
        /// <param name="script">The script.</param>
        public void RemoveExecutedScript(SqlScript script, Func<IDbCommand> dbCommandFactory)
        {
            EnsureTableExistsAndIsLatestVersion(dbCommandFactory);
            using (var command = GetRemoveScriptCommand(dbCommandFactory, script))
            {
                command.ExecuteNonQuery();
            }
        }
        ...
        protected IDbCommand GetRemoveScriptCommand(Func<IDbCommand> dbCommandFactory, SqlScript script)
        {
            var command = dbCommandFactory();
            ...
            command.CommandText = GetRemoveJournalEntrySql("@scriptName");
            command.CommandType = CommandType.Text;
            return command;
        }
        ...
        /// <param name="applied">Name of the applied param (i.e @applied)</param>
        /// <returns></returns>
        protected abstract string GetRemoveJournalEntrySql(string @scriptName);
